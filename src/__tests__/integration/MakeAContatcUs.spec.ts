...
test('should create a new contactUs', async () => {
  const makeAContact = new MakeAContactUs(new ContactUsDAOFaker(), new GeneralConfigDAOFaker(), new SendEmailGatewayFaker(), new EmailTemplateContact(), CONFIG_FAKER_COD)
...
test('should call ContactUsDAOFaker to save contact form', async () => {
  ...
  const makeAContactUs = new MakeAContactUs(contactUsDAOFaker, new GeneralConfigDAOFaker(), new SendEmailGatewayFaker(), new EmailTemplateContact(), CONFIG_FAKER_COD)
  await makeAContactUs.execute(body)
  expect(createSpy).toBeCalled()
})

test('should get email from generalConfig', async () => {
  const emailTemplate = new EmailTemplateContact()
  ...
  const makeAContactUs = new MakeAContactUs(new ContactUsDAOFaker(), new GeneralConfigDAOFaker(), sendEmailGateway, new EmailTemplateContact(), CONFIG_FAKER_COD)
  await makeAContactUs.execute(body)
